      ...
      <TouchableOpacity
        ...
        style={[
          styles.recipientCard,
          {
            ...
            borderLeftColor: item?.meta_info?.recepient_border_color,
          },
        ]}>
        <View
          style={[
        ...
        <View style={styles.flexOne}>
          <Text style={styles.recipientCardName} numberOfLines={1}>
            {item?.recepient_name}
          </Text>
          ...
          {item?.recepient_email && (
            <View style={styles.contactRow}>
              ...
              />
              <Text style={styles.recipientCardEmail} numberOfLines={1}>
                {item?.recepient_email}
              </Text>
            </View>
          )}
          ...
          {item?.recepient_phone && (
            <View style={styles.contactRow}>
              ...
              <Text style={styles.recipientCardEmail} numberOfLines={1}>
                {item?.recepient_phone_code}-{item?.recepient_phone}
              </Text>
            </View>
          )}
...
const RecipientFormModal = forwardRef(
  ({onClose, onSave, editData, openAddressBook}, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      populateFromAddressBook: async recipient => {
        ...
        if (method === 'sms' || method === 'email_and_sms') {
          const phoneCode = recipient?.recepient_phone?.includes('-')
            ? recipient?.recepient_phone?.split('-')[0]
            : '+1';
          const phone = recipient?.recepient_phone?.includes('-')
            ? recipient?.recepient_phone?.split('-')[1]
            : '';
          ...
          <View style={styles.formContainer}>
            ...
            <Controller
              ...
              render={({field: {onChange, value}, fieldState: {error}}) => (
                <AppInput
                  ...
                  error={error?.message}
                  leftIcon={User}
                  onRightIconPress={openAddressBook}
                  rightIcon={BookUser}
                />
              )}
            />
            ...
            {(method === 'email_and_sms' || method === 'sms') && (
              <View>
                ...
                {errors?.recepient_phone?.message && (
...
const AddRecipientScreen = ({navigation}) => {
  ...
  const enableSigningOrder = useAppSelector(
    state => state?.envelope?.set_signing_order,
  );
    ...
    ({item}) => {
      return (
        <View style={styles.fileItemError}>
          ...
          <TouchableOpacity
            onPress={() => handleValidateEmail(item)}
            style={{
              borderRadius: wp(8),
  ...
  const handleNext = async () => {
    ...
    if (!enableSigningOrder) {
      for (let recipient of recipients) {
        ...
        if (
          recipient &&
          (recipient.method === 'email' || recipient.method === 'email_and_sms')
        ) {
    ...
    let updatedRecipients = recipients.map((r, i) => {
      ...
      if (
        ...
        updatedRecipient.recepient_email = `${updatedRecipient.recepient_name}${i}@wesign.com`;
      }
      ...
      if (
        ...
        if (!emailCount.hasOwnProperty(email)) {
          ...
          updatedRecipient.recepient_email = `${name}+${emailCount[email]}@${domain}`;
        }
      }
  ...
  return (
    <CustomSafeAreaView>
      ...
      <View style={styles.container}>
        {recipients?.length > 0 && (
          <AppToggleButton
            containerStyle={styles.toggleBtnWrapper}
            size={40}
            label="Set Signing Order"
            value={enableSigningOrder}
            onToggle={v => dispatch(setSigningOrder(v))}
          />
        )}
      ...
      <AppBottomSheet
        ...
        <View style={{flex: 1}}>
          ...
          <Text
            ...
            We detected invalid email addresses in your list. Kindly update them
            before proceeding.
          </Text>
      ...
      <AppBottomSheet
        ...
        <View style={{flex: 1}}>
          ...
          <Text
            ...
            This envelope contains duplicate recipients at the same signing
            order position.
          </Text>
          ...
          <Text
            ...
            To keep the duplicates, edit the recipients list to set a signing
            order and adjust order as needed.
          </Text>

          <AppButton
            title="Enable Signing Order"
            onPress={() => {
