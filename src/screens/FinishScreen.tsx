...
const FinishScreen = ({ navigation }) => {
  ...
  const handleDayPress = useCallback(
    day => {
      ...
      setValue('expiry_date', selected.format('MM/DD/YYYY'), {
        shouldValidate: true,
      });
  ...
  const handleReminderDaySelect = useCallback(
    item => {
  ...
  const handleReminderCountSelect = useCallback(
    item => {
    ...
    ({ item }) => (
      <Pressable
        ...
        {selectedLanguage?.value === item?.value && (
          <Check size={fp(3)} color={Colors.success} />
        )}
      </Pressable>
    ),
  ...
  const renderReminderCountItem = useCallback(
    ({ item }) => (
      <Pressable
        ...
        {selectedNumberOfReminders === item && (
          <Check size={fp(3)} color={Colors.success} />
        )}
      </Pressable>
    ),
      ...
      <View style={styles.container}>
        <KeyboardAwareScrollView
          ...
          {isAutomaticRemindersChecked && (
            <View>
              ...
              <View style={styles.rowGap}>
                <Pressable
                  onPress={() => dayRef.current?.snapToIndex(0)}
                  style={[styles.dropdownPressable, styles.flexOne]}>
                  <Text style={styles.dropdownText}>{selectedDays?.label}</Text>
                  <ChevronDown size={fp(2)} color={Colors.text_primary} />
                </Pressable>
                <Pressable
                  onPress={() => reminderRef.current?.snapToIndex(0)}
                  style={[styles.dropdownPressable, styles.countPressable]}>
                  <Text style={styles.dropdownTextCenter}>
                    {selectedNumberOfReminders}
                  </Text>
                </Pressable>
              </View>
            </View>
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <TextInput
                    ...
                    onChangeText={text =>
                      handleExpiryDaysChange(text, onChange)
                    }
                  />
                )}
              />
          ...
          <View>
            ...
            <Pressable
              onPress={() => languageRef.current?.snapToIndex(0)}
          ...
          <Text style={styles.infoText}>
            Once the envelope is completed, all recipients will receive a copy
            of the completed envelope.
          </Text>
        </KeyboardAwareScrollView>
      </View>
