...
const ProfilePagerScreen = ({navigation}) => {
  ...
  const fetchAllAssetsAtOnce = async () => {
    ...
    try {
      ...
      if (
        ...
        sigRes.value.data.status_code === 200
      ) {
      ...
      if (
        ...
        initRes.value.data.status_code === 200
      ) {
      ...
      if (
        ...
        stampRes.value.data.status_code === 200
      ) {
      ...
      if (
        timezoneRes.status === 'fulfilled' &&
        timezoneRes.value.data.status_code === 200
      ) {
  ...
  const onPassSubmit = async data => {
    ...
    try {
      ...
      const response = await api.post('/auth/change/password', request_data);
  ...
  const allStates = useMemo(() => {
    const countryTemp = allCountries?.find(c => c.name === selectedCountry);
        ...
        <PagerView
          ...
          <View key="1">
            <ScrollView
              ...
              <View>
                ...
                <TouchableOpacity
                  style={styles.inputWrapper}
                  onPress={() => timezoneRef?.current?.snapToIndex(0)}>
                  <MapPin size={fp(2.2)} color="#94a3b8" />
                  <Text style={styles.pickerText} numberOfLines={1}>
          ...
          <View key="2">
            <ScrollView
              ...
              <AssetCard
                ...
                onEdit={() => setShowPrefilledSignatureModal(true)}
              />
              <AssetCard
                title="Initial"
                icon={Type}
                imageUrl={initBase64}
                onDelete={() => handleDelete('initial')}
                onEdit={() => setShowPrefilledInitialModal(true)}
              />
              <AssetCard
                title="Stamp"
                icon={StampIcon}
                imageUrl={stampBase64}
                onDelete={() => handleDelete('stamp')}
                onEdit={() => setShowPrefilledStampModal(true)}
              />
              <AssetCard
                title="Logo"
                icon={ImageIcon}
          ...
          <View key="3">
            <ScrollView contentContainerStyle={styles.scrollContent}>
              ...
              <TouchableOpacity
                style={styles.saveProfileBtn}
                onPress={handlePasswordSubmit(onPassSubmit)}>
