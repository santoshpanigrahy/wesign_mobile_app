...
const CanvasScreen = ({navigation}) => {
  ...
  // UI Modals
  const [showPrefilledSignatureModal, setShowPrefilledSignatureModal] =
    useState(false);
  const [showPrefilledInitialModal, setShowPrefilledInitialModal] =
    useState(false);
  ...
  const [showRadioButtonCreateModal, setShowRadioButtonCreateModal] =
    useState(false);
  ...
  const allNonRecipients = recipients?.filter(
    recp => recp?.action !== 'receive_copy',
  );
  ...
  const fetchSignature = async () =>
    (await api.get(`/api/signature?user=${id}`)).data;
  const fetchInitial = async () =>
    (await api.get(`/api/initial?user=${id}`)).data;
  ...
  const isTextStyleEligible = fieldName =>
    TEXT_STYLE_ELIGIBLE.includes(fieldName);
  ...
  const handleNext = () => {
    ...
    const temp_resp = recipients
      ?.filter(recp => recp.action !== 'receive_copy')
      .map(resp => resp.recepient_email);
      ...
      <Pressable
        ...
        <View
          style={[
            ...
            {
              borderColor: item?.meta_info?.recepient_border_color,
              backgroundColor: item?.meta_info?.recepient_border_color + '40',
            },
          ]}>
          <Text
            style={[
    ...
    <CustomSafeAreaView>
      <View style={styles.header}>
        <Pressable
          ...
          onPress={() => setShowDocuments(prev => !prev)}>
        ...
        {!im_signer && (
          <View style={styles.recipientsWrapper}>
            <Pressable
              ...
              <View
                style={[
                  styles.prefillToggleBtn,
                  {
                    borderColor: enablePrefilled
                      ? Colors.prefilled
                      : Colors.border,
                    backgroundColor: enablePrefilled
                      ? Colors.prefilled + '20'
                      : 'transparent',
                  },
                ]}>
                <UserPen
                  color={enablePrefilled ? Colors.prefilled : Colors.black}
                  size={fp(2)}
                />
                <Text
                  style={[
                    styles.prefillToggleText,
                    {
                      color: enablePrefilled
                        ? Colors.prefilled
          ...
          {selectedField && enableResize && (
            <View style={styles.sliderContainer}>
              ...
              <Slider
                ...
                maximumValue={max}
                onValueChange={val => {
                  if (typeof val === 'number') widthProgress.value = snap(val);
                }}
        ...
        <View style={styles.fieldOverlay}>
          ...
          <View style={[styles.metaHeader, {paddingTop: inset.top}]}>
            <View style={styles.metaHeaderTitleRow}>
              ...
              <Text style={styles.metaTitle}>
                {getFieldLabel(tempField?.field_name)}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.metaBody}>
            {!im_signer && (
              <>
                <View>
                  ...
                  <Pressable
                    onPress={() => recipientRef.current?.snapToIndex(0)}
                    style={styles.metaAssignBox}>
                    <View style={styles.metaAssignRow}>
                      <Text style={styles.metaAssignText}>
            ...
            {MetaComponent && (
              <MetaComponent
                field={tempField}
                setField={setTempField}
                updateFieldValue={updateFieldValue}
                openDateFormatPicker={() => dateRef.current?.snapToIndex(0)}
              />
            )}
            {isTextStyleEligible(tempField?.field_name) && (
              <TextStyleMeta
                field={tempField}
                setField={setTempField}
                openColorPicker={() => colorRef.current?.snapToIndex(0)}
                closeColorPicker={() => colorRef.current?.close()}
              />
            )}
          </View>
          {/* </View> */}
      ...
      <CreateRadioFieldModal
        visible={showRadioButtonCreateModal}
        onSave={data => {
      ...
      <AppBottomSheet
        ...
        <View style={styles.datePickerContainer}>
          {['MM/DD/YYYY', 'DD/MM/YYYY', 'YYYY/MM/DD'].map(format => (
            <Pressable
              key={format}
              style={styles.dateRowPressable}
              onPress={() => handleDateSelect(format)}>
              <View style={styles.dateRow}>
                <Text style={styles.dateText}>{format}</Text>
                {tempField?.date_format === format && (
                  <Check color={Colors.primary} size={fp(2.8)} />
                )}
              </View>
            </Pressable>
          ))}
