...
const SignaturePad = ({
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      await uploadSignature(base64Img);
    } catch (err) {
  ...
  const uploadSignature = async base64 => {
    ...
    try {
      ...
      const res = await api.post(`/api/signature`, requestData);

      let fieldData = {};
      if (res?.status === 200) {
        fieldData.id = res?.data?.signature?.signature_id;
        fieldData.base_url = res?.data?.signature?.signature;
      }
      ...
      Toast.show({
        type: 'success',
        text1: 'Success',
        text2: 'Signature uploaded successfully',
      });
    } catch (err) {
      Toast.show({
        type: 'error',
  ...
  return (
    <View style={{ flex: 1, padding: wp(4), backgroundColor: Colors.white }}>
      <View
        ...
        <Text style={styles.header}>Adopt you signature</Text>
      </View>
      <AppInput
        label="Full Name"
        ...
        {activeTab === 'Draw' && (
          <View style={{ flex: 1 }}>
            <View style={{ flexDirection: 'row', marginVertical: hp(2) }}>
              <Text
                style={{ fontFamily: Fonts.Regular, fontSize: fp(1.9), flex: 1 }}>
                Draw you Signature
              </Text>
              ...
              <TouchableOpacity
                ...
                <Text style={{ fontFamily: Fonts.SemiBold, color: Colors.error }}>
                  Clear
                </Text>
              </TouchableOpacity>
            </View>
        ...
        {activeTab === 'Upload' && (
          <View style={{ paddingVertical: hp(2) }}>
              ...
              {image && (
                <Pressable
                  ...
                  style={{
                    ...
                    right: 0,
                    top: 0,
                    backgroundColor: 'rgba(243, 166, 166,0.3)',
                    width: wp(10),
                    height: wp(10),
                    justifyContent: 'center',
                    alignItems: 'center',
            ...
            <Text
              ...
              initials will be the electronic representation of my signature and
              initials for all purposes when I (or my agent) use them on
              documents, including legally binding contracts - just the same as
              a pen-and-paper signature or initial.
            </Text>
          </View>
        )}
      </View>
