...
const ActionButton = memo(
  ({Icon, label, onPress, isOutline, disabled = false}) => (
    <TouchableOpacity
      onPress={onPress}
      activeOpacity={0.8}
      style={[styles.actionBtn, {opacity: disabled ? 0.3 : 1}]}
      disabled={disabled}>
      <View
        style={[styles.actionIconPill, isOutline && styles.actionPillOutline]}>
        <Icon
          color={isOutline ? Colors.blue : '#FFF'}
          size={ACTION_ICON_SIZE - 2}
        />
      </View>
      <Text style={[styles.actionLabel, isOutline && styles.labelOutline]}>
        {label}
      </Text>
    </TouchableOpacity>
  ),
...
const RecipientCard = memo(
  ({data, handleCopyEnvelopeLink, handleEnvelopeQrCode}) => {
    const initial = data?.recepient_name?.slice(0, 2);
    const isEmailDelivered =
      data.email_status === 'accepted' ||
      data.email_status === 'delivered' ||
      data.email_status === 'opened' ||
      data.email_status === 'clicked';
    const isEmailNotDelivered =
      data.email_status === 'rejected' || data.email_status === 'failed';
    const isEmailUnSubscribed =
      data.email_status === 'unsubscribed' ||
      data.email_status === 'complained';
        ...
        <View style={styles.headerRow}>
          <View
            style={[
              ...
              {backgroundColor: data?.meta_info?.recepient_border_color},
            ]}>
          ...
          {isEmailDelivered && (
            <View style={styles.deliveryRow}>
              ...
              <Text style={styles.deliveryText}>
                ...
                <Text style={styles.boldSuccess}>SUCCESS</Text>
              </Text>
            </View>
          )}

          {isEmailNotDelivered && (
            <View style={[styles.deliveryRow, {backgroundColor: '#f9eeee'}]}>
              <OctagonX size={wp(4.5)} color="#c52222" fill="#f9eeee" />
              <Text style={[styles.deliveryText, {color: '#c52222'}]}>
                Email Delivery Status -{' '}
                <Text style={styles.boldSuccess}>FAILED</Text>
              </Text>
            </View>
          )}

          {isEmailUnSubscribed && (
            <View style={[styles.deliveryRow, {backgroundColor: '#fcf6dc'}]}>
              <CircleAlert size={wp(4.5)} color="#f9ab00" fill="#fcf6dc" />
              <Text style={[styles.deliveryText, {color: '#f9ab00'}]}>
                Email Delivery Status -{' '}
                <Text style={styles.boldSuccess}>UNSUBSCRIBE</Text>
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.actionRow}>
          {data?.qr_code && (
            <TouchableOpacity
              ...
              activeOpacity={0.8}
              onPress={() => handleEnvelopeQrCode(data?.qr_code)}>
              <LinearGradient
          ...
          <TouchableOpacity
            onPress={() => handleCopyEnvelopeLink(data?.link)}
...
const SummaryScene = ({
  recipients,
  handleCopyEnvelopeLink,
  handleEnvelopeQrCode,
}) => (
  <ScrollView
    showsVerticalScrollIndicator={false}
    contentContainerStyle={styles.scenePadding}>
    <Text style={styles.sectionTitle}>Envelope Recipients</Text>
    {recipients.map((recip, index) => (
      <RecipientCard
        key={index}
        data={recip}
        handleCopyEnvelopeLink={handleCopyEnvelopeLink}
        handleEnvelopeQrCode={handleEnvelopeQrCode}
      />
    ))}
        ...
        return (
          <View key={index} style={{marginTop: hp(1)}}>
            <TouchableOpacity
              ...
              <Text style={[styles.titleText, {flex: 1}]} numberOfLines={1}>
                {doc.documentName}
              </Text>
              {isExpanded ? (
            ...
            {isExpanded &&
              doc?.urls?.map((urlj, imgIndex) => {
                return (
                  <TouchableOpacity
                    activeOpacity={0.8}
                    style={[styles.fileCard, {marginTop: hp(1)}]}
                    key={imgIndex}>
                    <Image
                      source={{uri: urlj?.url}}
                      resizeMode="contain"
...
const EnvelopeDetailsScreen = ({route}) => {
  ...
  const onDownloadCertificate = async () => {
    ...
    try {
      ...
      const response = await api.post(`/download/certificate`, requestData, {
      ...
      if (contentType.includes('application/json')) {
        ...
        throw new Error(
          parsedError?.message || 'Server failed to build certificate payload.',
        );
      }
  ...
  const printImages = async () => {
    try {
      ...
      console.log('605', images, documentImages);
  ...
  const downloadEnvelope = async () => {
    ...
    try {
      ...
      Toast.show({
        ...
        text2: 'Creating your zip package...',
      });
  ...
  const getDocDetails = async () => {
    try {
      ...
      const response = await api.get(`/api/envelope/details/${envelopeId}`);
      ...
      if (data.status_code === 200) {
        ...
        if (envelope_documents) {
          for (let index = 0; index < envelope_documents.length; index++) {
            ...
            const document_name = envelope_documents[index]['document_name'];
          ...
          {tabs.map((tab, index) => (
            <TouchableOpacity
              ...
              <View
                style={[
                  styles.tabIndicator,
                  activeTab === index && styles.activeIndicator,
                ]}
              />
            </TouchableOpacity>
          ))}
...
const styles = StyleSheet.create({
  ...
  actionIconPill: {
    width: ACTION_ICON_SIZE * 1.8,
    height: ACTION_ICON_SIZE * 1.8,
    borderRadius: wp(5),
