...
const SentScreen = ({navigation}) => {
  ...
  const downloadEnvelopeZip = async (requestData, subjectName) => {
    ...
    try {
      ...
      const response = await api.post(`/download/documents`, requestData, {
  ...
  const combinedDownload = async (requestData, fileName) => {
    try {
      ...
      const errorMessage =
        ...
        'Failed to connect to the download server.';
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await api.delete(`/api/envelope`, {
        data: {id: ids},
      });
      ...
      if (response.data.status_code === 200) {
        ...
        const finalData = data?.filter(
          envelope => envelope?.id !== selectedEnvelope?.id,
        );
        setData(finalData);
      } else {
        Toast.show({
  ...
  const fetchData = async (pageNum = 1, isRefreshing = false) => {
    ...
    try {
      ...
      const res = await api.post(
        `/api/sent/envelope?page=${pageNum}`,
        requestData,
      );
      ...
      const formatted = envelopes.map(item => ({
        ...
        recipients: (item.envelope_recepients || [])
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      ...
      console.log(
        `/api/envelope/sent/search?query=${search.trim()}&user=${userId}`,
      );

      const response = await api(
        `/api/envelope/sent/search?query=${search.trim()}&user=${userId}&from_date=&to_date=&status=&page=${pageNum}`,
      );
  ...
  const emailDownload = async () => {
    ...
    try {
      ...
      const response = await api.post(`/download/envelope`, request_data);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.inner}>
        ...
        {!firstLoading && data.length === 0 && (
          <NoDataFound message="You Did Not Send An Envelope To Anyone" />
        )}
      ...
      <AppBottomSheet ref={actionRef} snapPoints={['27%']} withCloseBtn={false}>
        <View style={{flex: 1}}>
          ...
          <AppActionButton
            btnText="Email Download"
            icon={MailOpen}
            onPress={emailDownload}
          />
          <AppActionButton
            btnText="Void"
            icon={Ban}
